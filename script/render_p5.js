...
function windowResized() {
    ...
    // 視窗改變時，微調箭頭比例（之後你也可以用 UI 控）
    forcePixelsPerNewton = constrain(Math.round(width / 160), 3, 10);
}

function keyPressed() {
    if (key === 'p' || key === 'P') FrictionSim.getState().paused = !FrictionSim.getState().paused;
    if (key === 'r' || key === 'R') FrictionSim.reset();
}
